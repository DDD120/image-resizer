// 키워드로 검색해서 나온 이미지를 원하는 사이즈로 리사이징하여 돌려주는 서버
...
// 이미지를 Unsplash에서 검색하거나, 이미 있다면 캐시된 이미지를 리턴
...
async function getCachedImageOrSearchedImage(query) {
  ...
  return {
    message: `Retruning new image: ${query}, width: ${size.width}, height; ${size.height}`,
    stream: fs.createReadStream(imageFilePath),
  };
}
